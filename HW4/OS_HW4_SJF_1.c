...
void insert_by_arrival(link *new_node) {
    link *curr = head->next;
    while (curr->next != tail && curr->data.execution < new_node->data.execution) {
        curr = curr->next;
    }
    if(curr->next==tail && curr->data.execution > new_node->data.execution){
        curr->prev->next = new_node;
        new_node->prev = curr->prev;
        new_node->next = curr;
        curr->prev = new_node;
    }else if(curr->next==tail && new_node->data.number == 1){
        curr->next = new_node;
        new_node->prev = curr;
        new_node->next = tail;
        tail->prev = new_node;
